export const LandingHero: React.FC = () => {
  const x = "";
  return (
    // <div className=" text-start md:text-center px-5">
    //   <h1 className="text-5xl lg:text-7xl font-extrabold text-slate-900 ">
    //     Applicant management at{" "}
    //     <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-blue-400">
    //       any scale
    //     </span>
    //   </h1>
    //   <p className="mt-6 text-xl lg:text-3xl text-slate-700 ">
    //     Plutomi streamlines your application process with automated workflows.
    //   </p>
    // </div>
    <div className="max-w-6xl text-start md:text-center px-5">
      <h1 className="text-5xl lg:text-7xl font-extrabold text-slate-900 ">
        Making Applicant Management Great Again{" "}
      </h1>
      <p className="mt-6 text-xl lg:text-3xl text-slate-700 ">
        Plutomi streamlines your application process with automated workflows at any scale.
      </p>
    </div>
  );
};
