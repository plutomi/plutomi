...
import { randomNumberInclusive } from './OLD/utils/randomNumberInclusive';
import { faker } from '@faker-js/faker';
import { nanoid } from 'nanoid';
import { items } from './OLD/utils/connectToDatabase';
...
const numberOfBatches = randomNumberInclusive(1, 5);
  ...
  try {
    ...
    for (let i = 0; i < numberOfBatches; i++) {
      ...
      for (let i = 0; i < applicantsPerBatch; i++) {
        ...
        const notesPerApplicant = [];
        const applicantFiles = [];
        for (let i = 0; i < randomNumberInclusive(3, 10); i++) {
          const applicantNote = {
            _id: `ORG#${orgForApplicant}#APPLICANT#${applicantId}#NOTE#${nanoid(10)}`,
            id: nanoid(10),
            orgId: orgForApplicant,
            type: 'note',
            applicantId,
            data: {
              note: faker.lorem.sentences(randomNumberInclusive(3, 100)),
              createdAt,
              updatedAt,
            },
            target: [{ id: applicantId, type: 'note' }],
          };
          notesPerApplicant.push(applicantNote);
        }

        for (let i = 0; i < randomNumberInclusive(1, 3); i++) {
          const applicantFile = {
            _id: `ORG#${orgForApplicant}#APPLICANT#${applicantId}#FILE#${nanoid(10)}`,
            id: nanoid(10),
            orgId: orgForApplicant,
            type: 'file',
            applicantId,
            target: [{ id: applicantId, type: 'file' }],
          };
          applicantFiles.push(applicantFile);
        }

        localBatch.push(newApplicant);
        notesPerApplicant.forEach((note) => localBatch.push(note));
        applicantFiles.forEach((note) => localBatch.push(note));
      }

      // OR query, note or file
      // { $or: [ { _id: /^ORG#plutomi#APPLICANT#RGHK3vNDJ4#NOTE/ }, { _id: /^ORG#plutomi#APPLICANT#RGHK3vNDJ4#FILE/ }] }

      // { $and: [ { target: { $elemMatch: { id: "plutomi-wjpIPl-xwt", type: "note" } } } ] }
      // Notes, index overloading
      // { _id: /^ORG#plutomi#APPLICANT#WxpYoa0Zox#NOTE#/ }

      // { _id: /^ORG#armstronginc#APPLICANT#rrg7jp4tTL#NOTE#/ }
      applicantsToCreate.push(localBatch);
    }
    // Active NYC Applicants using key overloading
    //  { $and: [ { target: { $elemMatch: { id: "plutomi-NYC", type: 'applicant' } } }, { target: { $elemMatch: { id: 'ACTIVE', type: 'Status' } } },  ] }

    // Using double target and
    //  { $and: [ { target: { id: "plutomi-NYC", type: 'applicant' }  }, { target:  { id: 'ACTIVE', type: 'Status' }  },  ] }
