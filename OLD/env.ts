...
const PORT = get('PORT').default(3000).required().asPort();

const NEXT_PUBLIC_WEBSITE_URL = get('NEXT_PUBLIC_WEBSITE_URL')
...
const MONGO_URL = get('MONGO_URL')
  ...
  .asString();
const ACM_CERTIFICATE_ID = get('ACM_CERTIFICATE_ID')
  .required(
    NEXT_PUBLIC_DEPLOYMENT_ENVIRONMENT === 'stage' || NEXT_PUBLIC_DEPLOYMENT_ENVIRONMENT === 'prod',
  )
...
const HOSTED_ZONE_ID = get('HOSTED_ZONE_ID')
  .required(
    NEXT_PUBLIC_DEPLOYMENT_ENVIRONMENT === 'stage' || NEXT_PUBLIC_DEPLOYMENT_ENVIRONMENT === 'prod',
  )
...
const LOGIN_LINKS_SECRET = get('LOGIN_LINKS_SECRET').required().asString();
const SESSION_SIGNATURE_SECRET_1 = get('SESSION_SIGNATURE_SECRET_1').required().asString();
